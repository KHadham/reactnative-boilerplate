...
import { BaseView, Input, Button, Text, Icon } from '@components/index';
...
import { LayoutAnimationHandler } from '@helper/animationUtils';
...
const App: React.FC<AppProps> = ({
  ...
  const ListEmptyComponent = () => (
    <View style={styles.emptyWrap}>
      ...
      <Text.Desc style={{ marginBottom: 40 }}>
  ...
  const renderItem = ({ item, index }: { item: Item, index: number }) => {
    return (
      <View key={`keys${index}`} style={{ marginBottom: spacing.md }}>
        <Text.SubTitle style={{ marginBottom: spacing.xxs }}>
          {item.title}
        </Text.SubTitle>
        <View
          style={{
      ...
      >
        ...
        <FlatList
          ...
          ListHeaderComponent={() => (
            ...
            >
              <Text.Title style={{ marginBottom: spacing.md }}>
                {title}
              </Text.Title>
              <Icon
                name={'backup-restore'}
