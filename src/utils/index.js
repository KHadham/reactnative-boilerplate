...
export const getNumberAllowNegative = (string = '') =>
  string.replace(/[^-?\.?\d*]+/g, '');
...
export const transformText = (
  ...
  if (params.numberOnly) {
    return getNumberOnly(value);
  }
  if (params.textAndSpaceOnly) {
    return getTextAndSpaceOnly(value);
  }
  return value;
};
...
export const isArrayDataNotEmpty = data =>
  Array.isArray(data) && data.length > 0;

export const strSplitJoin = (
  string = '',
  splitSeparator = '',
  joinSeparator = ''
...
export const formatPhoneNumber = (phoneNumberString = '') => {
  ...
  const matchPhoneNumber = cleaned.match(
    /^(0|62|)?(\d{3})(\d{4}|\d{3})(\d{4}|\d{3})$/
  );
  if (matchPhoneNumber) {
    const indoCode = '+62';
    return [
